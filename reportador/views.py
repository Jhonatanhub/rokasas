...
from .models import Cliente, InspeccionDetalle, InspeccionEvidencia, Inspeccion
...
def guardar_inspeccion(request):
    if request.method == "POST":
        ...
        try:
            ...
            # Mapeo de datos estructurados incluyendo TODAS las URLs de evidencia por bloque
            # (antes solo se tomaba la primera con .first(), perdiendo el resto de fotos/videos)
            detalles_pdf = []
            ...
            # Igual que con las evidencias: el logo debe quedar como URL absoluta
            ...
            datos_plano_pdf = {
                ...
                'fecha': str(inspeccion_creada.fecha),
                'hora': str(inspeccion_creada.hora.strftime('%H:%M') if inspeccion_creada.hora else ''),
            ...
            # base_url le permite a WeasyPrint resolver cualquier ruta relativa que
            # quede en el HTML (por si acaso), aunque aquí ya mandamos todo absoluto arriba.
            pdf_bytes = HTML(
                string=html_string,
            ...
            # Antes este texto estaba dentro de un f-string con sangría de código,
            # así que cada línea llegaba al correo con espacios de más al inicio.
            # Se arma explícitamente línea por línea para que el correo se vea limpio.
